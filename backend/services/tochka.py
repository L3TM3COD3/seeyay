...
from backend.firestore import get_tochka_hybrid_token
...
class TochkaClient:
    ...
    async def get_uapi_bearer_token(self) -> str:
        ...
        We obtain it once during OAuth callback and store it in Firestore.
        """
        if self._uapi_bearer_token and (time.time() + 30) < self._uapi_bearer_token_exp:
            return self._uapi_bearer_token

        # Prefer env var first (for manual overrides), then Firestore.
        env = os.getenv("TOCHKA_HYBRID_TOKEN")
        if env:
            token = env.strip()
            self._uapi_bearer_token = token
            # If token has exp claim, cache using it; otherwise just set 1 hour.
            try:
                decoded = jwt.decode(token, options={"verify_signature": False})
                exp = decoded.get("exp")
                self._uapi_bearer_token_exp = float(exp) if exp else (time.time() + 3600)
            except Exception:
                self._uapi_bearer_token_exp = time.time() + 3600
            return token

        hybrid = await get_tochka_hybrid_token()
        if not hybrid:
            raise Exception("Tochka Access Token Hybrid is not configured yet")
